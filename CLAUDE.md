...
| `analytics.js` | Google Analytics 4 (`GA_ID`) — تحميل gtag **مشروط بالموافقة** عبر `loadGtag()` (لا تتبّع قبل القبول) + `isGranted()` نقيّة. مربوط بـ`CookieConsent` (مفتاح `cp_consent_v2`). **مغطّى باختبارات** |
| `seo.js` | محرّك SEO لكل مسار (SPA): `applySeo`/`useSeo` (عنوان/وصف/canonical/OG/Twitter) + بُناة JSON-LD نقيّة `faqLd`/`breadcrumbLd`. **مغطّى باختبارات** |
