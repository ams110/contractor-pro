...
| `/admin` | `AdminDashboard` (lazy) — لوحة تحكّم المنصّة (مركز قيادة الأدمن): دخول مخصّص باسم مستخدم/كلمة مرور (أسرار Supabase)، يعرض إجمالي/جدد المستخدمين + الإيراد الشهري MRR + الاشتراكات + التجارب + توزيع الخطط + نمو التسجيلات + آخر المسجّلين. عبر edge `admin-stats` + RPC `admin_get_stats`. **مخفي — غير موصول بأي UI عام** |
...
| `admin-stats` | لوحة الأدمن (`/admin`): `{action:'login'}` يتحقّق من `ADMIN_USERNAME`/`ADMIN_PASSWORD` (أسرار) ويُصدر توكن HMAC موقّع صالح 8 ساعات؛ `{action:'stats'}` (بتوكن صالح بالـAuthorization) يستدعي RPC `admin_get_stats` بالـservice role ويُرجع إحصائيات المنصّة. **منفصل عن مصادقة Supabase تماماً** (`--no-verify-jwt`). أسرار: `ADMIN_USERNAME`/`ADMIN_PASSWORD` (+ اختياري `ADMIN_JWT_SECRET`) |
