...
| `/admin` | `AdminDashboard` (lazy) — لوحة تحكّم المنصّة (مركز قيادة الأدمن): دخول مخصّص باسم مستخدم/كلمة مرور (أسرار Supabase)، يعرض إجمالي/جدد المستخدمين + الإيراد الشهري MRR + الاشتراكات + التجارب + توزيع الخطط + نمو التسجيلات + آخر المسجّلين. عبر edge `admin-stats` + RPCs `admin_get_stats`/`admin_list_users`/`admin_user_detail`/`admin_broadcast`. تبويبات: نظرة عامة (نشطون + قمع تحويل + ARR) · **المستخدمون** (بحث + تفاصيل بعدّاداتهم + حظر + تغيير خطة + تمديد تجربة) · **رسالة جماعية** (إشعار لكل المستخدمين). يدعم **دخول بالبصمة (WebAuthn)** و**تغيير كلمة السر/اسم المستخدم**. **مخفي — غير موصول بأي UI عام** |
