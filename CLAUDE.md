...
| `scan-plan` | قراءة مخطط بناء/موقع بالرؤية (Claude Sonnet، rate-limited `scan_plan`) → `{ buildings:[{name,floors,unitsPerFloor}], confidence, notes }` لبناء «التوأم الرقمي» 3D تلقائياً في دفتر المشروع. يستعمل سرّ `ANTHROPIC_API_KEY` (نفس scan-receipt). مُضاف لـ`deploy.yml` (verify-jwt) |
