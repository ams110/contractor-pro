...
| `paddle.js` | تكامل Paddle: `getPaddle`، `PLAN_PRICES` (شهري) + `PLAN_PRICES_ANNUAL` (سنوي) + `pricesFor(cycle)`، `PLAN_META`، `openCheckout({plan,user,org,cycle})`، `openCustomerPortal`، `isCheckoutConfigured`، `manageSubscription` |
| `icount.js` | تكامل **iCount** (بوّابة دفع إسرائيلية — فيزا/ماستركارد + הוראת קבע): `openCheckout` (يطلب رابط صفحة دفع مستضافة من edge `icount-billing` ويعيد توجيه المتصفح) + `manageSubscription` (إلغاء أمر التكرار) + `isCheckoutConfigured`. كل الأسرار والمبالغ خادمية. خامل حتى `VITE_ICOUNT_ENABLED=true` |
| `billing.js` | **مبدّل بوّابة الدفع** — مصدر واحد للشاشات (`PricingPage`/`SettingsScreen`). يختار المزوّد بـ`VITE_PAYMENT_PROVIDER` (`paddle` افتراضي · `icount`). يصدّر `openCheckout`/`isCheckoutConfigured`/`manageSubscription`. إضافة/تبديل بوّابة = سطر env واحد بلا لمس الشاشات |
...
| `icount-billing` | بوّابة **iCount** (verify-jwt): `action:'checkout'` يُنشئ صفحة دفع iCount مستضافة (مع تفعيل التكرار הוראת קבע) ويعيد رابطها · `action:'cancel'` يلغي أمر التكرار. كل الأسرار والمبالغ خادمية. أسرار: `ICOUNT_CID/USER/PASS`، `ICOUNT_PRICE_{STARTER,PRO,BUSINESS}`، `ICOUNT_WEBHOOK_SECRET`، `APP_URL`. خامل (503) حتى تُضبط. ⚠️ أسماء نقاط نهاية/حقول iCount قد تختلف حسب إعداد حساب السليقة — أكّدها قبل الإطلاق |
| `icount-webhook` | استقبال إشعار iCount (IPN) عند الدفع/التجديد/الإلغاء ومزامنة `subscriptions`+`organizations` (نظير `paddle-webhook`). التحقّق بسرّ مشترك في رابط الـIPN (`?secret=ICOUNT_WEBHOOK_SECRET`). يطبّع حمولة JSON أو form-urlencoded عبر `extractFields()` |
...
`VITE_SUPABASE_URL` · `VITE_SUPABASE_ANON_KEY` · `VITE_VAPID_PUBLIC_KEY` · `VITE_PADDLE_CLIENT_TOKEN` · `VITE_PADDLE_ENVIRONMENT` · `VITE_PADDLE_PRICE_{STARTER,PRO,BUSINESS}` (شهري) · `VITE_PADDLE_PRICE_{STARTER,PRO,BUSINESS}_ANNUAL` (سنوي) · `VITE_SENTRY_DSN` · `VITE_PAYMENT_PROVIDER` (`paddle`/`icount`، افتراضي paddle) · `VITE_ICOUNT_ENABLED` (راية تفعيل iCount).
...
**أسرار Edge Functions (Supabase)**: `PADDLE_WEBHOOK_SECRET` · `PADDLE_PRICE_ID_{STARTER,PRO,BUSINESS}` (+`_ANNUAL`) · **iCount**: `ICOUNT_CID`/`ICOUNT_USER`/`ICOUNT_PASS` · `ICOUNT_PRICE_{STARTER,PRO,BUSINESS}` · `ICOUNT_WEBHOOK_SECRET` (+ اختياري `ICOUNT_API_BASE`) · `RESEND_API_KEY` · `SEND_EMAIL_HOOK_SECRET` · `EMAIL_FROM` · `APP_URL` · `ADMIN_USERNAME`/`ADMIN_PASSWORD` (+ اختياري `ADMIN_JWT_SECRET`) للوحة الأدمن `/admin`. (كلها خاملة بأمان حتى تُضبط — التطبيق يعمل بدونها؛ لوحة الأدمن تُرجع 503 حتى تُضبط بياناتها.)
