...
| `/admin` | `AdminDashboard` (lazy) — لوحة تحكّم المنصّة (مركز قيادة الأدمن): دخول مخصّص باسم مستخدم/كلمة مرور (أسرار Supabase)، يعرض إجمالي/جدد المستخدمين + الإيراد الشهري MRR + الاشتراكات + التجارب + توزيع الخطط + نمو التسجيلات + آخر المسجّلين. عبر edge `admin-stats` + RPC `admin_get_stats`. يدعم **دخول بالبصمة (WebAuthn)** و**تغيير كلمة السر/اسم المستخدم** من داخل اللوحة (زر إعدادات). **مخفي — غير موصول بأي UI عام** |
