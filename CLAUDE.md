...
| **Android (TWA)** | `android.yml` (يدوي، `workflow_dispatch`) يبني AAB+APK موقّعين عبر Bubblewrap من `android/twa-manifest.json` كـartifacts — **لا ينشر تلقائياً على Play**. ⚠️ التطبيق غلاف TWA يحمّل الموقع الحيّ: **التحديثات العادية (محتوى/ميزات) توصل المستخدم فوراً بلا APK**؛ الـAPK فقط لتغييرات الغلاف (أيقونة/اسم/splash/إشعارات) أو رفعة جديدة للمتجر. التفاصيل والأسرار في `android/README.md` |
