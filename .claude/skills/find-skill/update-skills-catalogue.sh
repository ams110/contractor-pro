...
CACHE_DIR=".claude/skills/find-skill/cache"
...
if [ -z "${SKILLSMP_API_KEY:-}" ]; then
    echo "⚠️  SKILLSMP_API_KEY not set. Add it to .claude/skills/find-skill/.env" | tee -a "${LOG:-/dev/null}"
    echo "   Run: echo 'export SKILLSMP_API_KEY=\"your_key\"' >> .claude/skills/find-skill/.env" | tee -a "${LOG:-/dev/null}"
fi
