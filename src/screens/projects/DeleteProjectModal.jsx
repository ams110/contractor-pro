...
import { Archive, Trash2, AlertTriangle, ReceiptText, CreditCard, Calendar, X } from 'lucide-react'
...
const L = {
  ar: {
    ...
    delete_desc:  'سيُحذف المشروع ولن تتأثر القبضات والمصاريف (تبقى بدون مشروع)',
    cancel:       'إلغاء',
  ...
  he: {
    ...
    delete_title: 'מחיקה סופית',
    delete_desc:  'הפרויקט יימחק אך הקבלות וההוצאות ישארו (ללא פרויקט)',
    cancel:       'ביטול',
...
export default function DeleteProjectModal({ project, userId, onArchive, onDelete, onClose, language = 'ar' }) {
  ...
  async function doDelete() {
    setPhase('signing')
    const sig = await bioConfirm(`حذف المشروع نهائياً: ${project.name}`, 'projects')
    if (!sig) { setPhase('choice'); return }
    setPhase('working')
    setBusy(true)
                ...
                <div style={{ padding: '12px 14px', borderRadius: 14, marginBottom: 16,
                  ...
                  {hasData && (
                    <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                      {counts.receipts > 0 && (
                        <Chip icon={ReceiptText} count={counts.receipts} label={t.receipts} color='#22C55E' />
                      )}
                      {counts.expenses > 0 && (
                        <Chip icon={CreditCard} count={counts.expenses} label={t.expenses} color='#EF4444' />
                      )}
                      {counts.workdays > 0 && (
                        <Chip icon={Calendar} count={counts.workdays} label={t.workdays} color={C.primary} />
                      )}
                    </div>
                  )}
                </div>
...
function ActionCard({ icon: Icon, iconColor, iconBg, title, desc, recommended, danger, onClick, disabled }) {
  return (
    <motion.button
      whileTap={{ scale: 0.98 }}
      onClick={onClick}
      ...
      style={{
        ...
        background: recommended ? `${C.primary}10` : danger ? 'rgba(239,68,68,0.07)' : 'rgba(255,255,255,0.04)',
        border: `1.5px solid ${recommended ? C.primary + '40' : danger ? 'rgba(239,68,68,0.3)' : C.border}`,
    ...
    >
      ...
      <div style={{ flex: 1 }}>
        <div style={{ fontSize: 14, fontWeight: 800, color: danger ? '#EF4444' : C.text, marginBottom: 3 }}>
          {title}
          {recommended && (
