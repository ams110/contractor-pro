        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
          ...
          <motion.div initial={{ y: '100%' }} animate={{ y: 0 }} exit={{ y: '100%' }}
            ...
            <div style={{ fontSize: 17, fontWeight: 900, color: C.text, marginBottom: 18 }}>
              {isEdit
                ? (language === 'he' ? 'ערוך פרויקט' : language === 'en' ? 'Edit Project' : 'تعديل المشروع')
                : (language === 'he' ? 'פרויקט חדש' : language === 'en' ? 'New Project' : 'مشروع جديد')}
            </div>

            {[
              { key: 'name',  label: language === 'he' ? 'שם' : language === 'en' ? 'Name' : 'اسم المشروع', type: 'text' },
              { key: 'price', label: language === 'he' ? 'מחיר' : language === 'en' ? 'Price (₪)' : 'السعر (₪)',  type: 'number' },
            ].map(({ key, label, type }) => (
            ...
            <div style={{ marginBottom: 12 }}>
              <label style={{ display: 'block', fontSize: 11, fontWeight: 700, color: C.textDim, marginBottom: 6 }}>
                {language === 'he' ? 'סטטוס' : language === 'en' ? 'Status' : 'الحالة'}
              </label>
            ...
            {error && (
              <div style={{ padding: '10px 14px', borderRadius: 12, background: 'rgba(239,68,68,0.12)', border: '1px solid rgba(239,68,68,0.3)', color: '#EF4444', fontSize: 12, marginBottom: 14 }}>
                {error}
              </div>
            )}

            <div style={{ display: 'flex', gap: 10, paddingBottom: 'max(24px, env(safe-area-inset-bottom, 24px))', position: 'sticky', bottom: 0, background: C.surface, paddingTop: 10 }}>
              ...
              <motion.button whileTap={{ scale: 0.97 }} onClick={handleSave} disabled={saving}
                ...
                {saving ? '...' : (isEdit ? (language === 'en' ? 'Save Changes' : 'حفظ التعديلات') : (language === 'he' ? 'שמור' : language === 'en' ? 'Save' : 'حفظ'))}
              </motion.button>
            </div>
          </motion.div>
        </motion.div>
...
function ProjectDetail({ project, workDays, expenses, clientReceipts, employees, payments, onClose, onUpdate, onDelete, addReceipt, updateReceipt, deleteReceipt, addExpense, deleteExpense, addWorkDay, deleteWorkDay, approveWorkDay, rejectWorkDay, expCats, payMethods, permissions, holidays, language }) {
  ...
  return (
    <div dir={dir} style={{ minHeight: '100vh', paddingBottom: 80 }}>
      {/* Header */}
      <div style={{ padding: '14px 16px', display: 'flex', alignItems: 'center', gap: 10, borderBottom: `1px solid ${C.border}`, background: C.surface, position: 'sticky', top: 0, zIndex: 10 }}>
        ...
        {confirmDel && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }}
              style={{ width: '100%', maxWidth: 320, background: C.surface, border: `1px solid rgba(239,68,68,0.3)`, borderRadius: 20, padding: '24px 20px' }}>
              <div style={{ fontSize: 16, fontWeight: 800, color: C.text, marginBottom: 10, textAlign: 'center' }}>
                {language === 'en' ? 'Delete Project?' : 'حذف المشروع؟'}
              </div>
              <div style={{ fontSize: 12, color: C.textDim, textAlign: 'center', marginBottom: 20 }}>
                {language === 'en' ? 'This action cannot be undone.' : 'هذا الإجراء لا يمكن التراجع عنه.'}
              </div>
