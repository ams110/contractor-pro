...
// ترجمة شريحة الفلتر للعرض فقط — القيمة تبقى عربية canonical (تُستعمل كمفتاح includes)
const FILTER_LABELS = {
  'الكل':    { he: 'הכל',         en: 'All' },
  'مواد':    { he: 'חומרים',      en: 'Materials' },
  'بضاعة':   { he: 'סחורה',       en: 'Goods' },
  'عدد':     { he: 'כלים',        en: 'Tools' },
  'وقود':    { he: 'דלק',         en: 'Fuel' },
  'إيجار':   { he: 'השכרה',       en: 'Rental' },
  'خدمات':   { he: 'שירותים',     en: 'Services' },
  'رواتب':   { he: 'משכורות',     en: 'Wages' },
  'تأمين':   { he: 'ביטוח',       en: 'Insurance' },
  'أخرى':    { he: 'אחר',         en: 'Other' },
}
function filterLabel(cat, language) {
  if (language === 'ar') return cat
  const e = FILTER_LABELS[cat]
  if (!e) return cat
  return language === 'he' ? e.he : (e.en ?? cat)
}
...
export default function ExpensesScreen({ expenses, projects, expCats, addExpense, deleteExpense, approveExpense, rejectExpense, employees, userId, permissions, showVatExpenses = true }) {
  ...
  async function scanReceipt() {
    ...
    try {
      ...
      setScanMsg('✓ ' + tl(language, 'تم استخراج البيانات تلقائياً', 'הנתונים חולצו אוטומטית', 'Data extracted automatically'))
  ...
  return (
    <div className="fade-up" style={{ padding:16, paddingBottom:100 }}>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:11 }}>
          ...
          <div>
            <div style={{ fontSize:18, fontWeight:900, color:C.text, letterSpacing:'-0.02em' }}>
              {tl(language, 'المصاريف', 'הוצאות', 'Expenses')}
            </div>
            <div style={{ fontSize:11, color:C.textDim, marginTop:1 }}>{approvedExpenses.length} {tl(language, 'سجل', 'רשומה', 'records')}</div>
          </div>
        </div>
            ...
            {showVAT ? (
              <>
                <div style={{ textAlign:'center' }}>
                  <div style={{ fontSize:10, color:C.textDim, fontWeight:600, marginBottom:4 }}>{tl(language, 'شامل الضريبة', 'כולל מע"מ', 'Incl. VAT')}</div>
                  <div style={{ fontSize:22, fontWeight:900, color:C.accent, fontFamily:'monospace' }}>{showAmounts ? `${fmt(total)}₪` : '•••'}</div>
                </div>
                <div style={{ width:1, background:C.border }} />
                <div style={{ textAlign:'center' }}>
                  <div style={{ fontSize:10, color:C.textDim, fontWeight:600, marginBottom:4 }}>{tl(language, 'صافي بدون ضريبة', 'נטו ללא מע"מ', 'Net excl. VAT')}</div>
                  <div style={{ fontSize:22, fontWeight:900, color:C.text, fontFamily:'monospace' }}>{showAmounts ? `${fmt(noVAT)}₪` : '•••'}</div>
                </div>
                <div style={{ width:1, background:C.border }} />
                <div style={{ textAlign:'center' }}>
                  <div style={{ fontSize:10, color:C.textDim, fontWeight:600, marginBottom:4 }}>{tl(language, 'ضريبة المدخلات', 'מס תשומות', 'Input VAT')}</div>
                  <div style={{ fontSize:22, fontWeight:900, color:C.warning, fontFamily:'monospace' }}>{showAmounts ? `${fmt(totalVATIn)}₪` : '•••'}</div>
                </div>
              </>
            ) : (
              <div style={{ textAlign:'center', width:'100%' }}>
                <div style={{ fontSize:10, color:C.textDim, fontWeight:600, marginBottom:4 }}>{tl(language, 'إجمالي المصاريف', 'סך ההוצאות', 'Total expenses')}</div>
        ...
        <div style={{ marginBottom:16 }}>
          <SectionLabel color={C.warning}>
            <span style={{ display:'inline-flex', alignItems:'center', gap:5 }}>
              <Hourglass size={13} strokeWidth={2.3} /> {tl(language, 'بانتظار الموافقة', 'ממתין לאישור', 'Awaiting approval')} ({pendingExpenses.length})
            </span>
          </SectionLabel>
              ...
              <PremiumCard key={ex.id} tone="fair" radius={18} padding="12px 14px" delay={Math.min(i * 0.03, 0.3)} style={{ marginBottom:10 }}>
                <div>
                  <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:10 }}>
                    <div>
                      <div style={{ fontSize:14, fontWeight:700, color:C.text }}>{tEnum(ex.category, language)}</div>
                    ...
                    <div style={{ marginBottom:10 }}>
                      {ex.receipt_url.match(/\.(jpg|jpeg|png|gif|webp)(\?|$)/i) ? (
                        <a href={ex.receipt_url} target="_blank" rel="noreferrer" onClick={e => { e.preventDefault(); openSignedUrl(ex.receipt_url) }} style={{ cursor:'zoom-in' }}>
                          <SignedImg src={ex.receipt_url} alt={tl(language, 'فاتورة', 'חשבונית', 'Invoice')} style={{ width:'100%', maxHeight:160, objectFit:'cover', borderRadius:10, border:`1px solid ${C.border}` }} />
                        </a>
                      ) : (
                        <a href={ex.receipt_url} target="_blank" rel="noreferrer" onClick={e => { e.preventDefault(); openSignedUrl(ex.receipt_url) }}
                          ...
                          <span style={{ fontSize:12, color:C.primary, fontWeight:600 }}>{tl(language, 'عرض الفاتورة (PDF)', 'הצג חשבונית (PDF)', 'View invoice (PDF)')}</span>
                        </a>
                      )}
                    </div>
                  )}
                  <div style={{ display:'flex', gap:8 }}>
                    <button onClick={() => approveExpense(ex.id)}
                      ...
                      <Check size={14} strokeWidth={2.5} /> {tl(language, 'موافقة', 'אישור', 'Approve')}
                    </button>
                    <button onClick={() => rejectExpense(ex.id)}
                      ...
                      <X size={14} strokeWidth={2.5} /> {tl(language, 'رفض', 'דחייה', 'Reject')}
                    </button>
                  </div>
                </div>
              </PremiumCard>
      ...
      <div style={{ position:'relative', marginBottom:10 }}>
        ...
        <input value={search} onChange={e => setSearch(e.target.value)} placeholder={tl(language, 'بحث بالمورد أو التصنيف أو المبلغ...', 'חיפוש לפי ספק, קטגוריה או סכום...', 'Search by vendor, category or amount...')}
      ...
      <div style={{ display:'flex', gap:6, overflowX:'auto', paddingBottom:4, marginBottom:16, scrollbarWidth:'none' }}>
        {FILTER_CATS.map(cat => (
          <FilterChip key={cat} label={filterLabel(cat, language)} active={filter === cat} onClick={() => setFilter(cat)}
      ...
      {sorted.length === 0 && pendingExpenses.length === 0
        ? (
            <div style={{ textAlign:'center', padding:'44px 0', color:C.textDim }}>
              ...
              <div style={{ fontSize:14, fontWeight:700, color:C.text, marginBottom:18 }}>{tl(language, 'ما في مصاريف', 'אין הוצאות', 'No expenses')}</div>
              <Btn onClick={() => setShowForm(true)}>{tl(language, '+ أضف مصروف', '+ הוסף הוצאה', '+ Add expense')}</Btn>
            </div>
          )
                ...
                <div style={{ display:'flex', alignItems:'center', gap:12 }}>
                  ...
                  <div style={{ flex:1, minWidth:0 }}>
                    <div style={{ display:'flex', alignItems:'center', gap:6, flexWrap:'wrap' }}>
                      ...
                      {showVAT && (() => {
                        ...
                        if (deduct === 0) return <span style={{ fontSize:9, fontWeight:700, color:C.textDim, background:'rgba(255,255,255,0.06)', padding:'1px 6px', borderRadius:5, border:`1px solid ${C.border}` }}>{tl(language, 'بلا ضريبة', 'ללא מע"מ', 'No VAT')}</span>
                    ...
                    <div style={{ fontSize:10, color:C.textMuted, marginTop:1 }}>
                      {fmtDate(ex.date)}{ex.payment_method ? ` • ${tEnum(ex.payment_method, language)}` : ''}
                    </div>
                  ...
                  <div style={{ display:'flex', flexDirection:'column', alignItems:'flex-end', gap:6 }}>
                    ...
                    <div style={{ display:'flex', gap:6, alignItems:'center' }}>
                      {ex.receipt_url && (
                        <a href={ex.receipt_url} target="_blank" rel="noreferrer" onClick={e => { e.preventDefault(); openSignedUrl(ex.receipt_url) }} style={{ textDecoration:'none', display:'flex', alignItems:'center', justifyContent:'center', width:28, height:28, borderRadius:8, background:`${C.secondary}18`, border:`1px solid ${C.secondary}33`, color:C.secondary }} title={tl(language, 'عرض الفاتورة', 'הצג חשבונית', 'View invoice')}><Paperclip size={12} strokeWidth={2} /></a>
                      )}
      ...
      {/* ── Modal إضافة مصروف ── */}
      <Modal open={showForm} onClose={() => { setShowForm(false); setReceiptFile(null); setPreview('') }} title={tl(language, 'مصروف جديد', 'הוצאה חדשה', 'New expense')}>
        ...
        {showVAT && form.amount && parseFloat(form.amount) > 0 && (
          <div style={{ marginTop:-8, marginBottom:14, padding:'8px 12px', background:`${C.border}33`, borderRadius:10, display:'flex', justifyContent:'space-between' }}>
            {(() => {
              ...
              const label  = deduct === 0
                ? tl(language, 'معفى من الضريبة', 'פטור ממע"מ', 'VAT exempt')
                : deduct < 1
                  ? `${tl(language, 'ضريبة ⅔', 'מע"מ ⅔', 'VAT ⅔')}: ${fmt(vatAmt)}₪`
                  : `${tl(language, 'ضريبة', 'מע"מ', 'VAT')}: ${fmt(vatAmt)}₪`
              return <>
                <span style={{ fontSize:11, color:C.textDim }}>{tl(language, 'صافي', 'נטו', 'Net')}: {fmt(Math.round(amt - vatAmt))}₪</span>
        ...
        <Input label={tl(language, 'التصنيف', 'קטגוריה', 'Category')} value={form.category} onChange={f('category')} options={expCats || EXP_CATS} required />

        {projects.length > 0 && (
          <div style={{ marginBottom:14 }}>
            <label style={{ fontSize:12, color:C.textDim, display:'block', marginBottom:6, fontWeight:600 }}>{tl(language, 'المشروع', 'פרויקט', 'Project')}</label>
        ...
        <Input label={tl(language, 'المحل / المورّد', 'חנות / ספק', 'Store / Vendor')} value={form.vendor}         onChange={f('vendor')} />
        <Input label={tl(language, 'طريقة الدفع', 'אמצעי תשלום', 'Payment method')}    value={form.payment_method} onChange={f('payment_method')} options={PAY_METHODS} />

        {/* رفع الفاتورة + AI scan */}
        <div style={{ marginBottom:14 }}>
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:6 }}>
            <label style={{ fontSize:12, color:C.textDim, fontWeight:600, display:'flex', alignItems:'center', gap:5 }}><Paperclip size={12} strokeWidth={2} /> {tl(language, 'فاتورة / إثبات (اختياري)', 'חשבונית / אישור (אופציונלי)', 'Invoice / proof (optional)')}</label>
            {receiptFile && receiptFile.type.startsWith('image/') && (
              <button onClick={scanReceipt} disabled={scanning}
                ...
                {scanning
                  ? <><Loader2 size={12} strokeWidth={2.5} style={{ animation:'spin .8s linear infinite' }} /> {tl(language, 'جاري المسح', 'סורק', 'Scanning')}</>
                  : <><Sparkles size={12} strokeWidth={2.3} /> {tl(language, 'AI مسح', 'סריקת AI', 'AI scan')}</>}
              </button>
            )}
          </div>
          ...
          {preview
            ? <div style={{ position:'relative' }}>
                <img src={preview} alt={tl(language, 'فاتورة', 'חשבונית', 'Invoice')} style={{ width:'100%', maxHeight:160, objectFit:'cover', borderRadius:12, border:`1px solid ${C.border}` }} />
            ...
            : <button onClick={() => fileRef.current.click()}
                ...
                <Camera size={16} strokeWidth={1.8} /> {tl(language, 'اضغط لرفع صورة الفاتورة', 'לחץ להעלאת תמונת חשבונית', 'Tap to upload invoice image')}
              </button>
          }
        ...
        <Btn onClick={save} full disabled={saving || !form.amount || !form.category}>
          <span style={{ display:'inline-flex', alignItems:'center', gap:6, justifyContent:'center' }}>
            ...
            {saving ? tl(language, 'جاري الحفظ...', 'שומר...', 'Saving...') : tl(language, 'أضف المصروف', 'הוסף הוצאה', 'Add expense')}
          </span>
        </Btn>
      </Modal>

      <ConfirmDialog open={!!confirmDel} onClose={() => setConfirmDel(null)}
        onConfirm={async () => { await deleteExpense(confirmDel); setConfirmDel(null) }} message={tl(language, 'حذف هالمصروف؟', 'למחוק את ההוצאה הזו?', 'Delete this expense?')} />
    </div>
  )
}
