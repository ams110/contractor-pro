...
import { C, GRAD, EXP_CATS, EXP_CAT_VAT, PAY_METHODS, BUSINESS_TYPES } from '../constants/index.js'
import { fmt, fmtDate, todayStr, validateExpense, calcExpenseVAT } from '../lib/helpers.js'
...
function VATBadge({ amount, category, date }) {
  const deduct = EXP_CAT_VAT[category] ?? 1.00
  if (deduct === 0) return <span style={{ fontSize:9, fontWeight:700, color:C.textDim, background:'rgba(255,255,255,0.06)', padding:'1px 6px', borderRadius:5, border:`1px solid ${C.border}` }}>لا מע"מ</span>
  const vatAmt = calcExpenseVAT(amount, category, date)
...
export default function ExpensesScreen({ expenses, projects, expCats, addExpense, deleteExpense, approveExpense, rejectExpense, employees, userId, permissions, businessType = BUSINESS_TYPES.MOREH }) {
  const showVat = businessType !== BUSINESS_TYPES.PATUR
  ...
  const totalVATIn  = approvedExpenses.reduce((s, e) => s + calcExpenseVAT(e.amount, e.category, e.date), 0)
    ...
    <div className="fade-up" style={{ padding:16, paddingBottom:100 }}>
                ...
                <div style={{ display:'flex', alignItems:'center', gap:12, padding:'12px 14px' }}>
                  ...
                  <div style={{ flex:1, minWidth:0 }}>
                    <div style={{ display:'flex', alignItems:'center', gap:6, flexWrap:'wrap' }}>
                      <div style={{ fontSize:14, fontWeight:700, color:C.text }}>{ex.category}</div>
                      {showVat && <VATBadge amount={ex.amount} category={ex.category} date={ex.date} />}
                    </div>
      ...
      <Modal open={showForm} onClose={() => { setShowForm(false); setReceiptFile(null); setPreview('') }} title="مصروف جديد">
        ...
        {showVat && form.amount && parseFloat(form.amount) > 0 && (() => {
          ...
          const vatAmt = calcExpenseVAT(amt, form.category, form.date)
          const deduct = EXP_CAT_VAT[form.category] ?? 1.00
          const label  = deduct === 0 ? 'معفى من مع"מ' : deduct < 1 ? `מע"מ ⅔: ${fmt(vatAmt)}₪` : `מע"מ: ${fmt(vatAmt)}₪`
          return (
            <div style={{ marginTop:-8, marginBottom:14, padding:'8px 12px', background:`${C.border}33`, borderRadius:10, display:'flex', justifyContent:'space-between' }}>
              <span style={{ fontSize:11, color:C.textDim }}>صافي: {fmt(amt - vatAmt)}₪</span>
