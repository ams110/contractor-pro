      ...
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:20 }}>
        ...
        <div style={{ display:'flex', gap:8, alignItems:'center' }}>
          {permissions?.isOwner && approvedExpenses.length > 0 && (
            <motion.button whileTap={{ scale: 0.93 }} onClick={async () => { const { ensureExportAllowed } = await import('../lib/exportGate.js'); if (!ensureExportAllowed('Excel')) return; const m = await import('../lib/export.js'); m.exportExpensesToExcel(approvedExpenses, projects) }}
