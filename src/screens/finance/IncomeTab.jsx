...
export default function IncomeTab({ userId, linkedProjects = [] }) {
  ...
  const [showArchived,  setShowArchived]  = useState(false)  // إظهار بيانات المشاريع المؤرشفة

  // قائمة المشاريع المعروضة في النموذج (غير مؤرشفة فقط)
  ...
  const filtered = useMemo(() => {
    let res = entries
    // إخفاء بيانات المشاريع المؤرشفة افتراضياً
    ...
    return res
  }, [entries, filterMonth, filterProj, showArchived, archivedProjectIds])
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <div>
              <div style={{ fontSize: 11, fontWeight: 800, color: '#818CF8' }}>
                {orphanedCount} قبضة بدون مشروع مرتبط
              </div>
              <div style={{ fontSize: 10, color: C.textDim, marginTop: 1 }}>
                هذه القبضات لن تظهر في إحصائيات أي مشروع
              </div>
            </div>
          </div>
          <button
      ...
      {archivedLinkedCount > 0 && !showArchived && (
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 12px', borderRadius: 12, marginBottom: 10, background: 'rgba(255,255,255,0.04)', border: `1px solid ${C.border}` }}>
          <span style={{ fontSize: 11, color: C.textDim }}>
            🗂 {archivedLinkedCount} قبضة من مشاريع مؤرشفة مخفية
          </span>
