...
export default function TeamScreen({
  ...
  const teamPulse = useMemo(() =>
    computeTeamPulse({ members: teamMembers, activity: allActivity }, language)
  , [teamMembers, allActivity, language])
  ...
  async function handleResetPass() {
    ...
    if (newPass.length < 6) return manager.setResetPassErr(tl(language, '6 أحرف على الأقل', '6 תווים לפחות', 'At least 6 characters'))
    setResetPassErr('')
  ...
  if (!permissions?.manageTeam) {
    return (
      <div style={{ textAlign: 'center', padding: '48px 16px' }}>
        ...
        <div style={{ fontSize: 14, color: C.textDim }}>{tl(language, 'ليس لديك صلاحية إدارة الفريق', 'אין לך הרשאה לנהל את הצוות', 'You do not have permission to manage the team')}</div>
      </div>
    )
  }
  ...
  return (
    <div>
      ...
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}
        ...
        <div>
          <div style={{ fontSize: 18, fontWeight: 900, color: C.text, letterSpacing: '-0.02em' }}>{tl(language, 'إدارة الفريق', 'ניהול הצוות', 'Team management')}</div>
          <div style={{ fontSize: 11, color: C.textDim, marginTop: 1 }}>{tl(language, 'الأعضاء والصلاحيات والوصول', 'חברים, הרשאות וגישה', 'Members, permissions and access')}</div>
        </div>
      </motion.div>
      ...
      {teamLoadError && (
        <div style={{ padding: '10px 14px', borderRadius: 12, marginBottom: 12, background: `${C.accent}15`, border: `1px solid ${C.accent}33`, fontSize: 11, color: C.accent, display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8 }}>
          ...
          {reloadTeam && <button onClick={reloadTeam} style={{ background: 'none', border: `1px solid ${C.accent}55`, borderRadius: 8, color: C.accent, fontSize: 11, padding: '4px 10px', cursor: 'pointer' }}>{tl(language, 'إعادة تحميل', 'טען מחדש', 'Reload')}</button>}
        </div>
      )}
      ...
      {teamMembers.length > 0 && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 8, marginBottom: 16 }}>
          {[
            { label: tl(language, 'إجمالي الأعضاء', 'סך החברים', 'Total members'), value: teamMembers.length, color: C.primary, tone: 'brand',     icon: Users },
            { label: tl(language, 'نشطون', 'פעילים', 'Active'),                     value: activeCount,         color: C.success, tone: 'excellent', icon: UserCheck },
            { label: tl(language, 'محجوبون', 'חסומים', 'Blocked'),                  value: blockedCount,        color: blockedCount > 0 ? C.accent : C.textDim, tone: blockedCount > 0 ? 'critical' : 'brand', icon: Ban },
          ].map((s, i) => (
      ...
      <div style={{ marginBottom: 12 }}>
        <Btn onClick={manager.openAddMember} full><span style={{ display: 'inline-flex', alignItems: 'center', gap: 6, justifyContent: 'center' }}><UserPlus size={15} strokeWidth={2.5} /> {tl(language, 'إضافة عضو جديد', 'הוספת חבר חדש', 'Add new member')}</span></Btn>
      </div>

      {/* ── Member list ── */}
      {teamMembers.length === 0 ? (
        <EmptyState icon={<Users size={40} color={C.textDim} strokeWidth={1.8} />} text={tl(language, 'لا يوجد أعضاء فريق بعد — أضف أول عضو الآن', 'אין עדיין חברי צוות — הוסף את החבר הראשון עכשיו', 'No team members yet — add your first member now')} />
      ) : (
        teamMembers.map(m => (
          <MemberCard
            key={m.id}
      ...
      {/* ── Add member modal ── */}
      <AddMemberModal manager={manager} onSubmit={handleAddMember} projects={projects} language={language} />

      {/* ── Edit perms panel ── */}
      <EditPermsPanel member={editingMember} manager={manager} onSave={handleUpdatePerms} projects={projects} language={language} />
        ...
        >
          <div style={{ width: '100%', maxWidth: 360, background: C.surface, borderRadius: 20, padding: 22, border: `1px solid ${C.borderMid}` }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 11, marginBottom: 14 }}>
              ...
              <div>
                <div style={{ fontSize: 15, fontWeight: 800, color: C.text }}>{tl(language, 'تغيير كلمة المرور', 'שינוי סיסמה', 'Change password')}</div>
            ...
            <input
              ...
              placeholder={tl(language, 'كلمة المرور الجديدة (6 أحرف على الأقل)', 'סיסמה חדשה (6 תווים לפחות)', 'New password (at least 6 characters)')}
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              <Btn onClick={handleResetPass} full disabled={manager.resetPassSaving}>
                {manager.resetPassSaving ? '...' : <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5, justifyContent: 'center' }}><Check size={14} strokeWidth={2.6} /> {tl(language, 'حفظ', 'שמירה', 'Save')}</span>}
              </Btn>
              <Btn onClick={() => manager.setResetPassTarget(null)} variant="outline" color={C.textDim} full>{tl(language, 'إلغاء', 'ביטול', 'Cancel')}</Btn>
            </div>
          </div>
        </div>
      )}
      ...
      <ConfirmDialog
        ...
        message={manager.confirmBlock?.blocked
          ? tl(language,
              ...
              `"${manager.confirmBlock?.name}" ייחסם מכניסה מיידית`,
              `"${manager.confirmBlock?.name}" will be blocked from access immediately`)
          : tl(language,
              `سيستعيد "${manager.confirmBlock?.name}" صلاحياته`,
              `"${manager.confirmBlock?.name}" יחזיר את ההרשאות שלו`,
              `"${manager.confirmBlock?.name}" will regain their permissions`)}
      />
      ...
      <ConfirmDialog
        ...
        message={tl(language,
          `سيتم حذف "${manager.confirmRemove?.display_name || manager.confirmRemove?.username}" نهائياً`,
          `"${manager.confirmRemove?.display_name || manager.confirmRemove?.username}" יימחק לצמיתות`,
          `"${manager.confirmRemove?.display_name || manager.confirmRemove?.username}" will be permanently deleted`)}
      />
    </div>
  )
}
