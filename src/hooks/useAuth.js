...
export function useAuth() {
  ...
  /**
   * تسجيل passkey جديدة على الجهاز الحالي (يحتاج المستخدم مسجّل دخوله بالباسورد مرة واحدة)
   * يستخدم supabase.auth.registerPasskey() — المفاتيح تُخزَّن في Supabase، لا على الجهاز
   */
   ...
   * يستخدم supabase.auth.signInWithPasskey() — Supabase يتحقق على السيرفر
   */
  async function signInWithPasskey() {
    try {
      const { data, error } = await supabase.auth.signInWithPasskey()
      if (error) throw error
      // تأكد من تعليم الجهاز بعد نجاح الدخول
      localStorage.setItem(PASSKEY_KEY, 'registered')
      return data
    } catch (e) {
  ...
  /** إلغاء البصمة من الجهاز (تمسح العلامة المحلية فقط — السجل في Supabase يُحذف من الإعدادات) */
