...
const IDEAS = [
  ...
  // 38 · موجة חשבוניות ישראל — العتبة نزلت لـ5,000₪ بحزيران 2026 (زاوية توقيت، CM-020)
  { tag: 'חשבוניות ישראל', tone: 'danger', screen: 'expenses', kw: 'فاتورة فوق 5,000₪', head: 'بلا رقم تخصيص؟ مرفوضة', sub: 'من حزيران 2026 كل فاتورة فوق 5,000₪ لازمها מספר הקצאה — كبلان بيأرشف فواتيرك مرتّبة وبيجهّزك قبل ما تتفاجأ.',
    blocks: [{ type: 'header', title: 'أرشيف الفواتير' }, { type: 'insights', rows: [{ icon: AlertTriangle, color: C.accent, text: 'العتبة نزلت لـ5,000₪ من حزيران 2026' }, { icon: CheckCircle2, color: C.success, text: 'كل فواتيرك مؤرشفة وجاهزة للفحص' }] }, { type: 'statRow', stats: [{ icon: Receipt, label: 'فواتير مؤرشفة', value: 148, tone: 'danger' }] }] },
]
...
const IDEAS_HE = [
  ...
  // 3 · מע"מ 18% (سلطة + خوف) — محرّك الضرائب الإسرائيلي · شاشة المصاريف
  { tag: 'מע"מ 18%', tone: 'danger', screen: 'expenses',
    kw: 'המע"מ עלה ל-18%.', head: 'רוב הקבלנים טועים.',
    sub: 'כבלאן מחשב מע"מ, מס הכנסה וביטוח לאומי אוטומטית — לפי החוק הישראלי. בלי טעויות, בלי הפתעות מרשות המסים.' },

  // 4 · مقارنة سعرية مع المحاسب (تثبيت سعر + خسارة) · شاشة الداشبورد (الفائز 32K)
  { tag: 'פחות מחצי מחיר', tone: 'gold', screen: 'dashboard',
    kw: 'רואה חשבון לא מכיר', head: 'אף עובד שלך.',
    sub: 'משלם לרואה חשבון אלפי שקלים בשנה — והוא לא יודע כמה אתה חייב לפועלים. כבלאן יודע הכול. 990 ₪ לשנה.' },

  // 5 · عصرة السيولة (الألم الموجود) · شاشة الداشبورد (التوقّع/الكاش)
  { tag: 'תזרים מזומנים', tone: 'cyan', screen: 'dashboard',
    kw: 'הלקוח משלם אחרי 70 יום.', head: 'המע"מ לא מחכה.',
    sub: 'כבלאן מראה כמה כסף נכנס, כמה יוצא וכמה באמת נשאר לך ביד — בזמן אמת, מהנייד.' },

  // 6 · ربحية المشروع (شفافية الربح) · شاشة المشاريع
  { tag: 'רווח לכל פרויקט', tone: 'success', screen: 'projects',
    kw: 'כמה אתה באמת', head: 'מרוויח מכל פרויקט?',
    sub: 'הכנסה פחות חומרים, פועלים והוצאות — רווח ואחוז אמיתי לכל פרויקט, אוטומטית. בלי הפתעות בסוף.' },

  // 7 · بطّل الورق/الإكسل (نفور الخسارة) · شاشة أيام العمل
  { tag: 'נגמר הנייר', tone: 'brand', screen: 'workdays',
    kw: 'עדיין רושם ימי עבודה', head: 'על דף?',
    sub: 'כל יום עבודה, שעות נוספות ומקדמה — בלחיצה מהנייד. הפועל מאשר, ואתה לא מאבד שקל.' },

  // 8 · دقّة الراتب (تجنّب الخلاف/الخطأ) · شاشة الرواتب
  ...
  // ═══ دفعة Content Machine (عمود B — CM-021…040) · أفكار #9–#28 ═══

  // 9 · مسح إيصال OCR (CM-021)
  { tag: 'סריקת חשבוניות', tone: 'brand', screen: 'expenses',
    kw: 'מצלם חשבונית —', head: 'וזהו.',
    sub: 'הבינה המלאכותית קוראת את הסכום, הספק, התאריך והקטגוריה לבד. בלי הקלדות, בלי ניירת שהולכת לאיבוד.' },

  // 10 · موجة חשבוניות ישראל 5,000₪ (توقيت — CM-022)
  { tag: 'חשבוניות ישראל', tone: 'danger', screen: 'expenses',
    kw: 'חשבונית מעל 5,000 ₪', head: 'בלי מספר הקצאה? לא עוברת.',
    sub: 'מיוני 2026 הרף ירד ל-5,000 ₪. כבלאן שומר כל חשבונית מסודרת בארכיון — מוכן מראש, בלי הפתעות מרשות המסים.' },

  // 11 · الذمّة الصافية (CM-023)
  { tag: 'שווי העסק', tone: 'premium', screen: 'dashboard',
    kw: 'כמה העסק שלך', head: 'באמת שווה?',
    sub: 'מזומן ועוד חובות של לקוחות פחות התחייבויות — השווי הנקי של העסק שלך, מחושב אוטומטית.' },

  // 12 · رادار التحصيل (CM-024)
  { tag: 'גבייה', tone: 'gold', screen: 'dashboard',
    kw: 'מי חייב לך כסף —', head: 'וכמה זמן כבר?',
    sub: 'כל חוב של לקוח לפי גיל: מי מאחר, כמה, ומי צריך תזכורת היום. הכסף שלך לא נשכח.' },

  // 13 · كشف الشذوذ (CM-025)
  { tag: 'התראות חכמות', tone: 'danger', screen: 'expenses',
    kw: 'הדלק קפץ 40% החודש.', head: 'שמת לב?',
    sub: 'כבלאן מזהה קפיצות והוצאות כפולות לבד ומתריע — לפני שזה הופך לבעיה.' },

  // 14 · السلف (CM-026)
  { tag: 'מקדמות', tone: 'gold', screen: 'workers',
    kw: 'כל מקדמה נרשמת.', head: 'שום שקל לא הולך לאיבוד.',
    sub: 'המקדמה יורדת אוטומטית מהשכר של הפועל. תמיד יודע בדיוק כמה נשאר לשלם.' },

  // 15 · التسجيل الجماعي (CM-027)
  { tag: 'רישום קבוצתי', tone: 'brand', screen: 'workdays',
    kw: 'כל האתר נרשם', head: 'בלחיצה אחת.',
    sub: 'רישום ימי עבודה לכל הפועלים בבת אחת — יום מלא, חצי יום, שעות נוספות. שנייה וגמרנו.' },

  // 16 · التقارير (CM-028)
  { tag: 'דוחות', tone: 'success', screen: 'payments',
    kw: 'דוח מלא לרואה החשבון —', head: 'בלחיצה.',
    sub: 'אקסל ו-PDF מסודרים: הוצאות, ימי עבודה, שכר ודוח פרויקט מלא. מוכן להדפסה.' },

  // 17 · واتساب (CM-029)
  { tag: 'וואטסאפ', tone: 'success', screen: 'payments',
    kw: 'תזכורת תשלום ללקוח —', head: 'בוואטסאפ, בלחיצה.',
    sub: 'הודעות מוכנות: שכר שולם, דוח חשבון לפועל, תזכורת ללקוח שמאחר. הכול מהאפליקציה.' },

  // 18 · الأمان (CM-030)
  { tag: 'אבטחה', tone: 'premium', screen: 'payments',
    kw: 'תשלום גדול?', head: 'רק עם טביעת אצבע.',
    sub: 'אישור ביומטרי לפעולות רגישות, נעילת מסך אוטומטית ויומן חתימות מלא. כמו בבנק.' },

  // 19 · بلا إنترنت (CM-031)
  { tag: 'עובד בלי אינטרנט', tone: 'cyan', screen: 'workdays',
    kw: 'אין קליטה באתר?', head: 'עובד גם בלי אינטרנט.',
    sub: 'הכול נשמר במכשיר ומסתנכרן לבד כשהרשת חוזרת. שום רישום לא הולך לאיבוד.' },

  // 20 · متعدّد المصالح (CM-032)
  { tag: 'כמה עסקים', tone: 'brand', screen: 'dashboard',
    kw: 'עוסק מורשה וגם חברה?', head: 'הכול במקום אחד.',
    sub: 'מחליף בין העסקים בלחיצה — לכל עסק המסים והדוחות שלו. עוסק פטור, מורשה או חברה.' },

  // 21 · صلاحيات الفريق (CM-033)
  { tag: 'הרשאות צוות', tone: 'premium', screen: 'projects',
    kw: 'המשגיח רואה', head: 'רק את הפרויקט שלו.',
    sub: 'מוסיף אנשי צוות עם הרשאות מדויקות: מי רואה סכומים, מי רק רושם ימים. הכול בשליטה שלך.' },

  // 22 · صحّة المشروع (CM-034)
  { tag: 'בריאות הפרויקט', tone: 'danger', screen: 'projects',
    kw: 'הפרויקט מפסיד —', head: 'ואתה תגלה אחרון?',
    sub: 'מדד בריאות לכל פרויקט: עלות, רווח, גבייה ולוח זמנים. התראה מוקדמת לפני שמאוחר מדי.' },

  // 23 · توقّع السيولة (CM-035)
  { tag: 'תחזית תזרים', tone: 'cyan', screen: 'dashboard',
    kw: 'כמה כסף יהיה לך', head: 'בעוד 30 יום?',
    sub: 'תחזית חכמה של המזומן שלך לשבועות הקרובים — עם טווח ביטחון ומונה ימי אוויר.' },

  // 24 · تجربة مجانية (CM-036)
  { tag: 'ניסיון חינם', tone: 'brand', screen: 'dashboard',
    kw: '14 יום חינם.', head: 'בלי כרטיס אשראי.',
    sub: 'הכול פתוח 14 יום — עובדים, שכר, מסים ופרויקטים. מתחילים בדקה, בלי התחייבות.' },

  // 25 · بصمة العامل (CM-037)
  { tag: 'פרופיל פועל', tone: 'cyan', screen: 'workers',
    kw: 'מי הפועל הכי אמין שלך?', head: 'יש תשובה.',
    sub: 'נוכחות, התמדה וביצועים לכל פועל — מדורג אוטומטית. מחליט על בסיס נתונים, לא תחושה.' },

  // 26 · نهاية السنة (CM-038)
  { tag: 'סוף שנה', tone: 'gold', screen: 'expenses',
    kw: 'סוף שנה', head: 'בלי פאניקה.',
    sub: 'כל ההכנסות וההוצאות מסודרות ומתויגות כל השנה. כשמגיע הדוח — לוחצים כפתור וזהו.' },

  // 27 · توثيق البضاعة (CM-039)
  { tag: 'תיעוד חומרים', tone: 'gold', screen: 'expenses',
    kw: 'כל שק מלט', head: 'מתועד — בלי ניירת.',
    sub: 'הפועל רושם את הסחורה מהפורטל שלו, ואתה רואה הכול בזמן אמת. שום חומר לא נעלם.' },

  // 28 · تعرفة الأعياد (CM-040)
  { tag: 'תעריפי חג', tone: 'gold', screen: 'workdays',
    kw: 'חג? שבת?', head: 'התעריף מחושב לבד.',
    sub: 'סוגי יום, חגים ושעות נוספות — הכול לפי חוק, אוטומטית. בלי ויכוחים בסוף החודש.' },
]
