...
// ─── الرافعة — تدور وتمدّ الكيبل بلا توقّف ────────────────────────────────────
function Crane({ shared }) {
  ...
  useFrame(() => {
    const t = shared.t
    if (slew.current) slew.current.rotation.y = Math.sin(t * 0.16) * 0.85 + 0.5
    if (trolley.current) trolley.current.position.x = 1.5 + Math.sin(t * 0.23 + 1) * 1.1
    const len = 2.7 + Math.sin(t * 0.4) * 1.0
...
const KEYS = [
  ...
  { p: 1.001, pos: new THREE.Vector3(2.0, 6.4, 10.4), look: new THREE.Vector3(2.6, 1.8, 1.8) }, // مشهد 3: مخطّط الأرباح
]
