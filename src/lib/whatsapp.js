...
export const waMessages = {
  ...
  /**
   * دعوة إحالة — من محاكاة الإحالة: 12/12 رفضوا «رسالة البيّاع»، فالقالب
   * قصير، مؤطَّر كهدية للصاحب (شهره المجاني)، **بلا أي ذكر لمكافأة المُرسِل**،
   * ومصمَّم ليُعدَّل بكلمات المُرسِل قبل الإرسال.
   */
  referralInvite: ({ url } = {}) =>
    `جرّب تطبيق كبلان — بستعمله لإدارة شغلي، بالعربي.\n` +
    `سجّل من هالرابط وبتاخد شهر مجاني:\n${url}`,

  /** إشعار الزبون باستلام دفعة + رقم الקבלה (ملف الקבלה يُرفق من نافذة الطباعة) */
  clientReceipt: ({ clientName = '', amount = 0, refNumber = '' } = {}) =>
    `مرحبا ${clientName}،\n`.replace('مرحبا ،', 'مرحبا،') +
    `تم استلام دفعتكم بمبلغ ₪${fmt(amount)}` +
    (refNumber ? ` — קבלה מס׳ ${refNumber}` : '') +
    `.\nشكراً لتعاملكم معنا.`,
}
